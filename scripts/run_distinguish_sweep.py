...
def main():
    parser = argparse.ArgumentParser(description="Run Distinguish Thoughts Sweep (Experiment C)")
    ...
    output_file = Path(args.output_dir) / "sweep_distinguish.jsonl"
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    print(f"Starting Distinguish Thoughts Sweep...")
    print(f"Concepts: {concepts}")
    print(f"Layers: {layers}")
    print(f"Strengths: {strengths}")
    print(f"Trials per condition: {args.n_trials}")
    
    total_trials = len(concepts) * len(layers) * len(strengths) * args.n_trials * 2 # x2 for prompt types
    ...
    with open(output_file, 'a') as f:
        for concept_name in concepts:
            ...
            if vector is None:
                ...
                pbar.update(len(layers) * len(strengths) * args.n_trials * 2)
                continue
            ...
            for layer in layers:
                ...
                for strength in strengths:
                    for _ in range(args.n_trials):
                        # Run both prompt types
                        for p_type in ["repeat", "think"]:
                            try:
                                result = run_distinguish_trial(
                                    model, tokenizer, vector, layer, strength, concept_name, prompt_type=p_type
                                )
